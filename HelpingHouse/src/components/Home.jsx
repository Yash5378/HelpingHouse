...
import { GetAllHelpingHouses } from "../Store/HelipngHouse/HelpingHouseAction";
...
const houses = [
  {
    ...
    verified: true,
    description:
      "A safe haven for abandoned and rescued dogs. We provide medical care, training, and find forever homes for every pup.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "Dedicated to providing dignity, comfort, and community for seniors who need support and companionship in their golden years.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "A loving home for children who need care, education, and the chance to dream big. Every child deserves a bright future.",
  ...
  },
  {
    ...
    verified: false,
    description:
      "Rescuing stray and abandoned cats across the city. We rehabilitate, vaccinate and rehome cats with loving families.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "A safe refuge for women and families escaping difficult situations. We provide shelter, counseling, and paths to independence.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "Fighting hunger one meal at a time. We distribute nutritious food to families, seniors, and children across the community.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "Providing housing, mental health support, and community to veterans who served our country and now need a hand up.",
  ...
  },
  {
    ...
    verified: false,
    description:
      "Empowering at-risk youth through education, mentorship, arts, and sports programs. Building tomorrow's leaders today.",
  ...
  },
  {
    ...
    verified: true,
    description:
      "Supporting individuals with physical and cognitive disabilities through adaptive care, therapy, and community inclusion programs.",
...
const houseTypes = [
  ...
  { label: "Dog Shelter", value: "dog_shelter" },
  { label: "Cat Shelter", value: "cat_shelter" },
  { label: "Children's House", value: "children_house" },
  { label: "Elder Care", value: "elder_care" },
  { label: "Women's Shelter", value: "women_shelter" },
  { label: "Food Bank", value: "food_bank" },
  { label: "Veterans' Home", value: "veterans_home" },
  { label: "Youth Center", value: "youth_center" },
...
const HOUSE_TYPE_IMAGES = {
  ...
  food_bank:
    "https://images.unsplash.com/photo-1542828925-6bb7b1b69fe0?auto=format&fit=crop&w=1200&q=80",
  veterans_home:
    "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=1200&q=80",
  youth_center:
    "https://images.unsplash.com/photo-1519241047957-be31d7379a5d?auto=format&fit=crop&w=1200&q=80",
  disability_support:
    "https://images.unsplash.com/photo-1515125520141-e03a8b4cd9dc?auto=format&fit=crop&w=1200&q=80",
  default:
    "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1200&q=80",
};

const getHouseImageByType = (typeKey) => {
  const key = String(typeKey || "").toLowerCase();
  if (key.includes("dog")) return HOUSE_TYPE_IMAGES.dog_shelter;
  if (key.includes("cat")) return HOUSE_TYPE_IMAGES.cat_shelter;
  if (key.includes("children")) return HOUSE_TYPE_IMAGES.children_house;
  if (key.includes("elder")) return HOUSE_TYPE_IMAGES.elder_care;
  if (key.includes("women")) return HOUSE_TYPE_IMAGES.women_shelter;
  if (key.includes("food")) return HOUSE_TYPE_IMAGES.food_bank;
  if (key.includes("veteran")) return HOUSE_TYPE_IMAGES.veterans_home;
  if (key.includes("youth")) return HOUSE_TYPE_IMAGES.youth_center;
  if (key.includes("disability")) return HOUSE_TYPE_IMAGES.disability_support;
  return HOUSE_TYPE_IMAGES.default;
};

function HouseCard({ house, viewMode, onFavorite, isFavorite, onView }) {
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        boxShadow:
          "0 2px 16px rgba(59,130,246,0.07), 0 1px 4px rgba(0,0,0,0.05)",
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow =
          "0 12px 40px rgba(59,130,246,0.14), 0 4px 16px rgba(0,0,0,0.08)";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      >
        <img
          src={getHouseImageByType(house.typeKey)}
          alt={house.name}
          style={{
            width: "100%",
            height: "100%",
        ...
        >
          ...
          {house.verified && (
            <Tooltip title="Verified Organization">
              <CheckCircleFilled
                style={{ color: "#3b82f6", fontSize: 15, flexShrink: 0 }}
              />
            </Tooltip>
          )}
        </div>
...
export default function Home() {
  ...
  const helpingHouseState = useSelector((state) => state.helpingHouse || {});
  const {
    getAllHelpingHousesLoading,
    getAllHelpingHousesData,
    getAllHelpingHousesError,
  } = helpingHouseState;

  const fetchedHouses = getAllHelpingHousesData?.data || [];

  const mappedHouses = fetchedHouses.length
    ? fetchedHouses.map((h) => ({
        ...h,
        type: h.type || h.ngoType || "NGO",
        typeKey:
          h.typeKey ||
          (h.ngoType ? h.ngoType.toLowerCase().replace(/\s+/g, "") : "all"),
        tagColor: h.tagColor || "#3b82f6",
        tagBg: h.tagBg || "rgba(59,130,246,0.08)",
        emoji: h.emoji || "🏠",
        volunteers: h.volunteers || 0,
        rating: h.rating || 0,
        verified: h.isApproved || false,
        needs: h.needs || [],
      }))
    : [];

  const fetchHouses = (type = "all", page = 1) => {
    const params = new URLSearchParams();
    if (type && type !== "all") {
      params.append("ngoType", type);
    }
    params.append("page", page);
    params.append("limit", pageSize);
    dispatch(GetAllHelpingHouses(token, params.toString()));
  };
  ...
  const handlePageChange = (page) => {
    setCurrentPage(page);
    fetchHouses(selectedType, page);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  // Filter + sort
  const apiLoaded = typeof getAllHelpingHousesData !== "undefined";
  const dataSource = apiLoaded ? mappedHouses : houses;
  ...
  const totalCount =
    getAllHelpingHousesData?.pagination?.total ?? filtered.length;
  const currentHouses = apiLoaded
    ? filtered
    : filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize);

  const stats = [
    {
      label: "Helping Houses",
      value: totalCount,
      icon: "🏠",
      color: "#3b82f6",
    },
    {
      ...
      value: dataSource.reduce((s, h) => s + (h.volunteers || 0), 0),
      icon: "🤝",
      color: "#10b981",
    },
    {
      label: "Verified Orgs",
      value: dataSource.filter((h) => h.verified).length,
      icon: "✅",
      color: "#8b5cf6",
    },
    {
      label: "Categories",
      value: houseTypes.length - 1,
      icon: "📂",
      color: "#f59e0b",
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            >
              Connect with verified shelters, orphanages, elder homes, and
              community centers that need your support.
            </Text>
          </div>
          ...
          >
            <Input
              ...
              prefix={
                <SearchOutlined style={{ color: "#93c5fd", fontSize: 16 }} />
              }
              style={{
                flex: 1,
            ...
            <Col xs={12} sm={6} key={i}>
              <div
                style={{
                  ...
                  padding: "18px 20px",
                  boxShadow:
                    "0 4px 20px rgba(59,130,246,0.1), 0 1px 6px rgba(0,0,0,0.05)",
                  border: "1px solid #e8f0fe",
                  display: "flex",
                  alignItems: "center",
                  gap: 14,
      ...
      {/* ── Main Content ──────────────────────────────────── */}
      <div
        style={{ maxWidth: 1200, margin: "0 auto", padding: "36px 32px 60px" }}
      >
        {/* ── Filter + Sort bar ── */}
        <div
          style={{
            ...
            {houseTypes.slice(0, 6).map((t) => (
              <button
                ...
                style={{
                  ...
                  border:
                    selectedType === t.value ? "none" : "1.5px solid #e2e8f0",
                  borderRadius: 10,
                  padding: "7px 16px",
                  fontSize: 13,
                  fontWeight: 600,
        ...
        >
          <Text style={{ color: "#64748b", fontSize: 14 }}>
            ...
            helping house{filtered.length !== 1 ? "s" : ""}
            {selectedType !== "all" && (
              <>
                {" "}
                in{" "}
        ...
        {getAllHelpingHousesLoading ? (
          ...
          <div style={{ textAlign: "center", padding: 40 }}>
            <Text type="danger">{getAllHelpingHousesError}</Text>
          </div>
